...
| `metaInboxEnabledByAgency` | **Beta master switch** for the Facebook Messenger + Instagram DM unified-inbox channels (both ride one Meta connection, so they flip together) | `false` | No tear-down — the consumer slices (Meta inbound webhook, send route, connect/settings UI, channel badge) aren't built yet, so while off the feature is **inert and invisible** everywhere. Ships off so an agency lights it up only for a sub-account that has a connected Meta account and volunteers to beta-test a channel that can't be fully self-tested. Re-enabling once the channels ship lights them up instantly. |
...
3. **Agency-only PATCH** — `PATCH /api/agency/sub-accounts/[id]/feature-gates` accepts any subset of `{emailDomainEnabled, apiAccessEnabled, broadcastsEnabled, outboundVoiceEnabled, whatsappEnabled}` (only fields you send get applied). Auth-gated to `subAccountRole === "agencyOwner"`.
...
src/
  ...
  lib/
    ...
    firestore/           CRUD helpers per collection (contacts, deals, tasks, events, forms, quotes, activities, users, mail, web-chat-sessions)
...
- **Connection model:** per-sub-account `subAccountDoc.metaConfig` ({@link MetaConfig} in [src/types/tenancy.ts](src/types/tenancy.ts)) holds the Page id/name, long-lived Page access token (stored like `twilioConfig.authToken`), and linked IG account id/handle. Populated by the OAuth callback; null until connected.
- **Connect flow:** [src/app/api/sub-accounts/[id]/meta/connect/route.ts](src/app/api/sub-accounts/[id]/meta/connect/route.ts) (admin-only; re-checks the agency gate + `metaAppConfigured()`, then redirects to Facebook Login with an HMAC-signed `state`) → [.../meta/callback/route.ts](src/app/api/sub-accounts/[id]/meta/callback/route.ts) (verifies state, exchanges the code for a Page token, subscribes the Page to our webhook, writes `metaConfig`) → redirects back to Settings with a `?meta=…` status the card toasts. Disconnect: `DELETE .../meta` clears `metaConfig` (best-effort webhook unsubscribe; history kept).
...
- **Lib:** [src/lib/comms/meta.ts](src/lib/comms/meta.ts) — `metaAppConfigured()`, OAuth URL + signed `state`, `X-Hub-Signature-256` verification, code→token exchange, `me/accounts` page listing, page (un)subscribe, profile-name lookup, and `sendMetaMessage()` (Messenger via Page id / Instagram via IG id, `messaging_type: "RESPONSE"`). Graph version pinned `v21.0`.
- **Settings UI:** [src/components/settings/sub-account-meta-section.tsx](src/components/settings/sub-account-meta-section.tsx) under Settings → Messaging. **Self-gating** — early-returns `null` unless the caller is admin AND the gate is on, so it appears in a sub-account only when unlocked. Shows Connect / connected Page + IG handle / Disconnect, plus the webhook URL + OAuth redirect URI to register in the Meta app.
