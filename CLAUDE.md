...
**`sharedSmsAllowed` — inverse-polarity gate (opt-out, not opt-in).** Every other row above defaults OFF and the agency turns a feature ON. This one is backwards: it defaults to `true` (undefined reads as allowed) so every sub-account can send/receive SMS on the agency's shared Twilio number out of the box — matching a "give free functionality now" onboarding flow. Setting it to `false` for one specific sub-account (via the same `SubAccountManageDialog`, "Require own Twilio account" checkbox) cuts *only that sub-account* off from the shared sender: `getTwilioForSubAccount()` in `lib/comms/twilio.ts` throws a friendly error on the next outbound send until that sub-account configures its own dedicated Twilio number (Settings → SMS). This lets an agency onboard sub-accounts on the shared number, then later "graduate" specific ones onto their own account without touching anyone else. Distinct from `AgencyDoc.sharedSmsAllowed` (`lib/agency/policy.ts::agencyAllowsSharedSms()`), which is a blanket agency-wide kill switch affecting every sub-account without dedicated Twilio at once. No tear-down — the block only affects future sends; existing message history and dedicated-mode config (if any) are untouched.
...
| `review_rating_request` | Texts the contact asking them to rate 1-5 instead of sending the Google review link directly, reusing the sub-account's Settings → Messaging → Review requests config (review URL + templates) — no config of its own. Requires dedicated Twilio (reply interception) + a configured review URL; fails soft (skips to `next`) otherwise. A 4-5 reply gets the Google link; 1-3 gets the internal-feedback message + a follow-up Task — this send/don't-send decision is always a hard rule on the number, never an AI judgment call. Same gate as the Settings-driven rating request (`lib/reviews/rating-reply.ts`) — see that section below for the hold/confirm state machine. The run pauses (`status: "waiting"`) until the reply resolves or 7 days elapse, whichever is first; `lib/workflows/engine.ts::resumeReviewRatingRun()` is the early-resume path, called from `rating-reply.ts` the moment a rating commits. As soon as the ask sends, texts the owner immediately ("A review request was sent to {name} ({phone})."), separate from the eventual outcome notification a downstream `notify_owner_sms` sends. |
| `review_rating_reminder` | One-time follow-up for `review_rating_request`'s 7-day no-reply timeout — no config of its own, place it right after that step. A transparent passthrough if the contact already replied (`{{reviewOutcome}}` context already set). Otherwise: texts the owner the window lapsed with no response, resends the same ask (bypassing the normal re-ask cooldown — this is an explicit one-time reminder, not a repeat auto-send), texts the owner the reminder went out, and re-arms another 7-day wait so a reply to the reminder is caught the same way. |
...
An inbound from an **unknown** number (dedicated mode only) auto-creates a Contact — phone-first, `source: "sms"`, named `"Lead text #N"` via [src/lib/leads/lead-label.ts](src/lib/leads/lead-label.ts) until it gets a real name — via `createContactServerSide` so `contact.created` fires, mirroring the WhatsApp inbound webhook below. Same one exception: a never-seen number whose *first* message is STOP/START is dropped without minting a contact. Shared mode can't do this (no subAccountId to scope a new contact to) and still drops an unmatched number.
...
**Reply delay (`AiChannelConfig.replyDelaySec`, 0-60s, default 0)** — a purely cosmetic "typing delay" so the bot doesn't reply the instant a message arrives. The LLM call always happens immediately; only the SEND is deferred. When set, `maybeRespondWithAi()` enqueues a QStash callback (`POST /api/webhooks/ai/delayed-send`, signature-verified, public path) instead of sending inline — the inbound webhook still returns immediately either way, so Twilio's own webhook timeout is never at risk. `lib/comms/ai/respond.ts::sendAndPersistReply()` is the shared "actually send + persist" tail both the immediate and delayed paths call, so there's exactly one send/persist implementation. Token usage is counted once, at generation time, regardless of which path sends the reply. If QStash isn't configured or the enqueue fails, falls back to sending immediately rather than dropping the reply.
...
- **Inbound:** [src/app/api/webhooks/twilio/whatsapp/inbound/route.ts](src/app/api/webhooks/twilio/whatsapp/inbound/route.ts) — dedicated-only (no shared-env fallback). Strips the `whatsapp:` address prefix, resolves the sub-account by `whatsappFromNumber`, enforces the agency gate, validates the Twilio signature, flips `whatsappOptedOut` on STOP/START, stores inbound to `contacts/{id}/whatsappMessages/{sid}`, and dispatches `maybeRespondWithAi({channelId: "whatsapp"})`. Same as SMS dedicated mode (see above), an inbound from an **unknown** number auto-creates a Contact (phone-first, name from the Twilio `ProfileName` param, `source: "whatsapp"`, via `createContactServerSide` so `contact.created` fires) and then replies — so a public "Contact Us on WhatsApp" link captures brand-new leads. The lone exception: a never-seen number whose *first* message is STOP/START is dropped without minting a contact.
